...
export default function DiscoveryDetail({ discoveryId }: DiscoveryDetailProps) {
  ...
  const approachText = APPROACH_MAP[displayDiscovery.type] || "Collect fresh qualitative evidence and run cross-reference reviews."
    ...
    <div className="h-full flex flex-col bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl overflow-hidden shadow-2xl">
      ...
      <div className="p-6 flex-1 overflow-y-auto space-y-6">
        <div>
          <span className={`text-[9px] font-mono font-bold px-2.5 py-0.5 rounded border uppercase tracking-wider ${
            displayDiscovery.severity > 0.8
              ? 'bg-red-50 text-red-600 border-red-900/60'
              : 'bg-amber-950/30 text-amber-455 border-amber-900/60'
          }`}>
            {label}
          </span>
          
          <p className="mt-4 text-sm text-slate-800 font-light leading-relaxed">
            {displayDiscovery.description}
          </p>
        </div>

        {/* Severity Details */}
        <div className="border-y border-[#E2E8F0]/60 py-5 flex justify-between items-center font-mono text-xs">
          <div>
            <span className="text-[10px] text-slate-500 uppercase font-semibold block mb-1">
              Risk Level
            </span>
            <span className="text-lg font-extrabold text-slate-900">
              {Math.round(displayDiscovery.severity * 100)}%
            </span>
          </div>
            ...
            <div className="space-y-3 text-xs leading-relaxed">
              {displayDiscovery.metadata.claim_1 && (
                <div className="p-4 bg-slate-50 border border-[#E2E8F0] rounded-xl">
                  <span className="font-mono text-[9px] text-blue-600 font-semibold block mb-1.5 uppercase">Hypothesis A:</span>
                  <p className="text-slate-800 font-light">{displayDiscovery.metadata.claim_1}</p>
                </div>
              )}
              {displayDiscovery.metadata.claim_2 && (
                <div className="p-4 bg-slate-50 border border-[#E2E8F0] rounded-xl">
                  <span className="font-mono text-[9px] text-red-600 font-semibold block mb-1.5 uppercase">Hypothesis B:</span>
                  <p className="text-slate-800 font-light">{displayDiscovery.metadata.claim_2}</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="p-5 rounded-xl border bg-slate-50/50 border-slate-200 hover:border-slate-300 transition-all">
          ...
          <div className="space-y-3 text-xs leading-relaxed">
            <div>
              <span className="font-semibold text-slate-700 block mb-1">Suggested Investigation Approach</span>
              <p className="text-slate-600 font-light">{approachText}</p>
            </div>
            <div>
              <span className="font-semibold text-slate-700 block mb-1">Expected Validation Evidence</span>
        ...
        {/* Actions */}
        <div className="pt-6 border-t border-[#E2E8F0]/60 flex gap-3">
          <button
            onClick={() => {
            ...
            className="flex-grow py-2.5 px-4 bg-slate-100 border border-[#E2E8F0] hover:border-slate-300 hover:bg-slate-200 disabled:opacity-50 text-slate-700 font-semibold rounded-lg text-xs transition-colors"
          >
